...
def main():
    ...
    if os.path.exists(previous_scores_file):
        ...
        f1_constructors = {name: Constructor(value['drivers'], value['WCC_points']) for name, value in
                           imported_data.items()}
    else:
        # F1 teams:
    ...
    with open(previous_scores_file, "w+") as save_data_file:
        json.dump(
            {constructor_name: constructor.to_dict() for constructor_name, constructor in f1_constructors.items()},
            save_data_file)
